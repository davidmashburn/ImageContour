...
import numpy as np
from np_utils import addBorder,groupByFunction,compose
...
def GetDividingLinesAndValuePairsFromArray(arr,axis,wrap):
    ...
    whwhp = getWhWhp(arr,axis)
    edgesAxis      = GetDividingLinesFromArray(arr,axis,whwhp)
    valuePairsAxis = GetValuePairsFromArray(arr,axis,wrap,whwhp)
    return edgesAxis,valuePairsAxis

def GetEdgesAndValuePairsFromArray(arr):
    evpX = GetDividingLinesAndValuePairsFromArray(arr,0,wrap=True)
    evpY = GetDividingLinesAndValuePairsFromArray(arr,1,wrap=False)
    edges,valuePairs = map( np.concatenate,zip(evpX,evpY) )
    return edges,valuePairs

def GetEdgeGroups(edges,valuePairs):
    return groupByFunction( zip(edges.tolist(),np.sort(valuePairs).tolist()),
